...
export class XMFile {
  ...
  async solveDependencie(dependencie: any, root: any) {
    ...
    const test = await getPackageInfo(dependencie, { paths: root })
    try {
      meta.resolve = resolveModule(dependencie, { paths: root })
      ...
      if (packageJson?.packageJson?.['web-types']) {
        const file = await vscode.workspace.fs.readFile(vscode.Uri.file(path.join(root, 'node_modules', dependencie, packageJson['web-types'])))
      ...
      if (packageJson?.packageJson?.module) {
        const packageIndexPath = path.join(root, 'node_modules', dependencie, packageJson?.packageJson?.module)
      ...
      // if (['js'].includes(path.extname(meta.resolve).replace('.', ''))) {
      //     console.log('js', meta.resolve, packageJson)
      ...
      //     return meta
      // }
      if (['js', 'jsx', 'ts', 'tsx', 'vue'].includes(path.extname(meta.resolve).replace('.', ''))) {
        const json = await this.solveJSFile({
      ...
      // const staticExports = await getExportsStatic(dependencie, { url: root })
      // meta.content = 'unknow'
      // meta.children = staticExports?.map?.(name => ({
      //   title: name,
      //   id: path.join(root, 'node_modules', dependencie, name),
      //   FileType: 'Export',
      // }))

      const runtimeExports = await getExportsRuntime(dependencie, { url: root })
      meta.content = 'unknow'
      meta.children = runtimeExports?.map?.(name => ({
        title: name,
        id: path.join(root, 'node_modules', dependencie, name),
        FileType: 'Export',
      }))
